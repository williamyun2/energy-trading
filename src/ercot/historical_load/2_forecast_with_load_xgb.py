...
print("=" * 70)
print("ERCOT DAM Forecasting - XGBoost with COAST Load (LEAKAGE-FREE)")
print("=" * 70)
...
print("\n" + "=" * 70)
print("XGBOOST PERFORMANCE (LEAKAGE-FREE, WITH LOAD)")
print("=" * 70)
...
# Feature importance
importances = pd.DataFrame({"feature_name": features, "importance": model.feature_importances_})
...
test_with_preds[["datetime", "price", "prediction", "error"]].to_csv(
    results_dir / "xgb_load_leakage_free_predictions.csv", index=False
)
...
with open(results_dir / "xgb_load_leakage_free_results.json", "w") as f:
    json.dump(results, f, indent=2)

print("\n" + "=" * 70)
print("✅ LEAKAGE-FREE XGBoost (WITH LOAD) COMPLETE")
print("=" * 70)
